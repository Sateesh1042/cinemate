...
const MovieDetail = ({title}) => {
  ...
  useEffect(() => {
    ...
    const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=d91b3ca2a70c569ed279a9f0378bf590&language=en-US`);
          ...
          <div className="max-w-2xl text-gray-700 text-lg dark:text-white">
            ...
            {movie.genres? (<p className="my-7 flex flex-wrap gap-2">
              {movie.genres.map((genre)=>(
                  <span className="mr-2 border border-gray-200 rounded dark:border-gray-600 p-2" key={genre}>{genre.name}</span>
              ))}
